...
这是用于 iPhone/iPad 主屏幕的质粒工具，支持 SnapGene `.dna`、GenBank、FASTA、项目 JSON 和 Sanger `.ab1` 多视图测序比对 / 构建验证。
...
4. 程序会自动裁剪低质量两端、判断正向/反向互补，并以多视图联动显示全局差异概览、Reference/Sample 主比对、差异表和构建解释。
5. 若质粒含有 CDS、tag、linker、同源臂、引物或酶切位点注释，差异会自动关联到这些层，并提示沉默、错义、终止、frameshift 或 junction 风险。
6. v2.5.1 起，连续 indel 会按一个事件解释；例如完整 codon `TAG` 缺失会显示为 `TAG` deletion / in-frame codon deletion，而不是拆成多个单碱基缺失。
